...
class TlsStreamSettings extends XrayCommonClass {
  ...
  static fromJson(json = {}) {
    ...
    return new TlsStreamSettings(json.serverName, certs, json.alpn);
  }
...
TlsStreamSettings.Cert = class extends XrayCommonClass {
  constructor(
    ...
    this.cert =
      certificate instanceof Array ? certificate.join("\n") : certificate;
...
class Inbound extends XrayCommonClass {
  constructor(
    ...
    this.settings = ObjectUtil.isEmpty(settings)
      ? Inbound.Settings.getSettings(protocol)
      : settings;
    ...
    switch (this.protocol) {
      case Protocols.VMESS:
        if (this.settings.vmesses[index]._expiryTime != null)
          return (
            this.settings.vmesses[index]._expiryTime < new Date().getTime()
          );
        return false;
      case Protocols.VLESS:
        if (this.settings.vlesses[index]._expiryTime != null)
          return (
            this.settings.vlesses[index]._expiryTime < new Date().getTime()
          );
        return false;
      default:
        return false;
  ...
  genVmessLink(address = "", remark = "", clientIndex = 0) {
    ...
    if (network === "tcp") {
      ...
      if (type === "http") {
        ...
        let index = request.headers.findIndex(
      ...
      path = ws.path;
      let index = ws.headers.findIndex(
        (header) => header.name.toLowerCase() === "host"
      );
      if (index >= 0) {
    ...
    switch (type) {
      case "tcp":
        ...
        if (tcp.type === "http") {
          ...
          const index = request.headers.findIndex(
            (header) => header.name.toLowerCase() === "host"
          );
          if (index >= 0) {
      ...
      case "ws":
        ...
        const index = ws.headers.findIndex(
          (header) => header.name.toLowerCase() === "host"
        );
        if (index >= 0) {
  ...
  genTrojanLink(address = "", remark = "") {
    let settings = this.settings;
    return `trojan://${settings.clients[0].password}@${address}:${
      this.port
    }#${encodeURIComponent(remark)}`;
  }
...
Inbound.VmessSettings = class extends Inbound.Settings {
  ...
  static fromJson(json = {}) {
    return new Inbound.VmessSettings(
      ...
      ObjectUtil.isEmpty(json.disableInsecureEncryption)
        ? false
...
Inbound.TrojanSettings.Client = class extends XrayCommonClass {
  ...
  static fromJson(json = {}) {
    return new Inbound.TrojanSettings.Client(json.password, json.flow);
  }
};

Inbound.TrojanSettings.Fallback = class extends XrayCommonClass {
  constructor(name = "", alpn = ["http/1.1"], path = "", dest = "", xver = 0) {
    super();
    this.name = name;
    this.alpn = ["http/1.1"];
    this.path = path;
  ...
  static fromJson(json = []) {
    ...
    for (let fallback of json) {
      fallbacks.push(
        new Inbound.TrojanSettings.Fallback(
          fallback.name,
          fallback.alpn,
          fallback.path,
          fallback.dest,
